...
(()=>{
    for(var category of categories)
        category[1] = String.fromCodePoint(parseInt(category[1], 16)); 
    for(var groupName in emotes) {
        var group = emotes[groupName];
        for(var subgroupName in group) {
            var subgroup = group[subgroupName];
            for(var emote of subgroup)
